...
function image_to_field(path::String; solid_val=0.0, fluid_val=1.0, threshold=0.5)
    ...
    field = Field(permutedims(field_array)) # Transpose image to allign with matrix coordinates
    smoothened_fieled = smooth_field(field) # Smoothen field using gausian filter to introduce a gradient in the fluid-solid interface
    
    return smoothened_fieled 
end

# TODO: Verify smoothing function works as intended
function smooth_field(field::Field{Float64}; sigma::Float64=1.0)
    smoothed_array = imfilter(field.a, Kernel.gaussian(sigma))
    return Field(smoothed_array)
end
...
# Constructor that creates PixelBody from physical bounds
...
Convert physical coordinates to grid coordinates (i.e. pixel coordiantes) using the window mapping.
...
function sdf(body::PixelBody, x, t=0.0; kwargs...)
    ...
    grid_coords = physical_to_grid(body, x) # TODO: Verify if this is necessary
    i, j = grid_coords[1], grid_coords[2]
    
    # Get field value at this point (0 = solid, 1 = fluid) # TODO: Verify the field is indeed defined like this
...
function create_example_body()
    ...
    smoothened_field = smooth_field(field)

    return PixelBody(smoothened_field)
end
